 ...
 * @param {Array} props.toc - 目錄結構陣列
 * @param {Array} props.tags - 標籤樹陣列
 * @param {boolean} props.loading - 是否載入中
 * @param {Function} props.onTagsUpdate - 標籤更新回撥
 */
...
export default function DomainAnalysis({ projectId, toc = '', loading = false }) {
  ...
  const saveTagChanges = async updatedTags => {
    console.log('儲存標籤更改:', updatedTags);
    setSaving(true);
    try {
