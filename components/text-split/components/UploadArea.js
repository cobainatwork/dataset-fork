...
export default function UploadArea({
  ...
  const handleDrop = e => {
    ...
    if (files && files.length > 0) {
      // 構造一個模擬的 event 以複用 onFileSelect
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            <Typography variant="h6" color="primary" sx={{ fontWeight: 'bold' }}>
              {t('textSplit.dragToUpload', { defaultValue: '拖拽檔案到此處上傳' })}
            </Typography>
          </Box>
        </Box>
      )}
      ...
      <Tooltip
        title={!selectedModel?.id ? t('textSplit.selectModelFirst', { defaultValue: '請先在右上角選擇模型' }) : ''}
      >
        <span>
          <Button
            component="label"
      ...
      {files.length > 0 && (
        <Box sx={{ mt: 3, width: '100%' }}>
          ...
          <Box sx={{ mt: 3, display: 'flex', justifyContent: 'center' }}>
            <Tooltip
              title={
                !selectedModel?.id ? t('textSplit.selectModelFirst', { defaultValue: '請先在右上角選擇模型' }) : ''
              }
            >
              <span>
                <Button
