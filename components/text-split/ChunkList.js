...
export default function ChunkList({
  ...
  // 褰撶瓫閫夋潯浠跺彉鍖栨椂錛屾竻闄や笉鍦ㄧ瓫閫夌粨鏋滀腑鐨勯€変腑欏?
  useEffect(() => {
  ...
  const handleBatchEdit = async editData => {
    try {
      ...
      if (result.success) {
        ...
        throw new Error(result.message || '鎵歸噺緙栬緫澶辮觸');
      }
    } catch (error) {
      console.error('鎵歸噺緙栬緫澶辮觸:', error);
      // 榪欓噷鍙互娣誨姞閿欒鎻愮ず
    } finally {
    ...
    try {
      ...
      for (const chunkId of selectedChunks) {
        try {
          ...
          console.error(`鍒犻櫎鏂囨湰鍧?${chunkId} 澶辮觸:`, error);
          failCount++;
        }
      }
      ...
      if (failCount === 0) {
        ...
        console.log(`刪除完成：成功 ${successCount} 個，失敗 ${failCount} 個`);
      }
