...
export default function GaPairsManager({ projectId, fileId, onGaPairsChange }) {
  ...
  const [backupGaPairs, setBackupGaPairs] = useState([]); // 備份狀態
  ...
  const loadGaPairs = async () => {
    try {
      ...
      if (result.success) {
        ...
        setBackupGaPairs([...loadedData]); // 建立備份
  ...
  const generateGaPairs = async () => {
    try {
      ...
      if (result.success) {
        ...
        setBackupGaPairs([...updatedGaPairs]); // 更新備份
  ...
  const handleGaPairChange = (index, field, value) => {
    ...
    setGaPairs(updatedGaPairs);
    // 不立即呼叫 onGaPairsChange，等使用者點選儲存時再呼叫
  };
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      {/* Generate GA Pairs Section - 只在沒有GA對時顯示 */}
      {gaPairs.length === 0 && (
        <Card sx={{ mb: 3 }}>
