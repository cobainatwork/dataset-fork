...
export default function TextSplitPage({ params }) {
  ...
  // 娑撳﹣鞝墮崠鍝勭厵閻ㄥ嫬鐫嶅鈧?閹舵ê褰旈悩鑸碘偓?
  const [uploaderExpanded, setUploaderExpanded] = useState(true);

  // 閺傚洨灝為崚妤勩€?FileList)鐏炴洜銇氱€電鉲藉鍡欏Ц閹?
  ...
  const fetchUploadedFiles = async (page = currentPage, fileName = searchFileName) => {
    try {
      ...
      toast.error(error.message || '閼懼嘲褰囬弬鍥︽閸掓銆冩徑杈Е');
    } finally {
      setLoading(false);
    }
  };

  // 閸掔娀婭庨弬鍥︽綰喛顓葷€電鉲藉鍡欏Ц閹?
  ...
  const confirmDeleteFile = async () => {
    ...
    try {
      ...
      toast.success(
        t('textSplit.deleteSuccess', { fileName: fileToDelete.fileName }) || `刪除 ${fileToDelete.fileName} 成功`
      );
    } catch (error) {
      console.error('刪除檔案出錯:', error);
      toast.error(error.message || '刪除檔案失敗');
    } finally {
      setLoading(false);
      setFileToDelete(null);
    }
  ...
  // 文字塊資料重新整理：初始化 + 檔案處理任務狀態變化
  useEffect(() => {
    fetchChunks('all');
  }, [fetchChunks, taskFileProcessing]);

  // 檔案列表重新整理：檔案分頁、搜尋關鍵詞變化時觸發
  useEffect(() => {
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 8, position: 'relative' }}>
      ...
      >
        <IconButton
          ...
          sx={{
            ...
            boxShadow: 1,
            mr: uploaderExpanded ? 1 : 0 // 鐏炴洖鞝戦弮鑸靛瘻闁筋喕綆ｉ梻瀵告殌閻愬綊媯跨捄?
          }}
          size="small"
        >
        ...
        {/* 閺傚洨灝為崚妤勩€冮幍鈺佺潔閹稿鎸抽敍灞肩矌閸︺劋絎傞柈銊ュ隘閸╃喎鐫嶅鈧弮鑸墊▔緇€?*/}
        {uploaderExpanded && (
          <IconButton
            color="primary"
            ...
            size="small"
            title={t('textSplit.expandFileList') || '擴充套件檔案列表'}
          >
            <FullscreenIcon />
          </IconButton>
        )}
      ...
      >
        ...
        <DialogContent dividers sx={{ p: 3 }}>
          {/* 濮濄倕顦╂徑宥囨暏 FileUploader 緙佸嫪嬈㈡稉顓犳畱 FileList 闁劌鍨?*/}
          <Box sx={{ minHeight: '80vh' }}>
            ...
            <FileList
              ...
              onRefresh={fetchUploadedFiles} // 嫻肩娀鈧帒鍩溳弬鏉垮毐閺?
              isFullscreen={true} // 閸︺劌顕拠嬋囶攱娑擃厾些闂勩倝鐝惔錒傛閸?
            />
          </Box>
        </DialogContent>
      </Dialog>
