    ...
    <Card sx={imageStyles.filterCard}>
      <CardContent>
        <Box sx={imageStyles.filterContent}>
          {/* 搜尋框 */}
          <TextField
            placeholder={t('images.searchPlaceholder', { defaultValue: '搜尋圖片名稱...' })}
          ...
          <FormControl size="small" sx={imageStyles.filterSelect}>
            <InputLabel>{t('images.hasQuestions', { defaultValue: '問題狀態' })}</InputLabel>
            <Select
              value={hasQuestions}
              onChange={e => onHasQuestionsChange(e.target.value)}
              label={t('images.hasQuestions', { defaultValue: '問題狀態' })}
            >
              <MenuItem value="all">{t('common.all', { defaultValue: '全部' })}</MenuItem>
              <MenuItem value="true">{t('images.withQuestions', { defaultValue: '有問題' })}</MenuItem>
              <MenuItem value="false">{t('images.withoutQuestions', { defaultValue: '無問題' })}</MenuItem>
            </Select>
          </FormControl>

          {/* 資料集狀態篩選 */}
          <FormControl size="small" sx={imageStyles.filterSelect}>
            <InputLabel>{t('images.hasDatasets', { defaultValue: '資料集狀態' })}</InputLabel>
            <Select
              value={hasDatasets}
              onChange={e => onHasDatasetsChange(e.target.value)}
              label={t('images.hasDatasets', { defaultValue: '資料集狀態' })}
            >
              <MenuItem value="all">{t('common.all', { defaultValue: '全部' })}</MenuItem>
