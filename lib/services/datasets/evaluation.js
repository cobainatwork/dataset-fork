 ...
 */
export async function evaluateDataset(projectId, datasetId, model, language = 'zh-TW') {
  try {
    // 1. 獲取資料集資訊
 ...
 */
export async function batchEvaluateDatasets(projectId, datasetIds, model, language = 'zh-TW', onProgress = null) {
  const results = [];
  let successCount = 0;
  let failedCount = 0;
