...
function extractProductCode(text) {
  if (!text) return [];
  const out = new Set();
  const matches = text.matchAll(/[（(]\s*([A-Z]{2,5}\d{3,6}(?:\s*\/\s*[A-Z]{2,5}\d{3,6})*)\s*[)）]/g);
  for (const m of matches) {
    const inner = m[1];
    for (const code of inner.split('/')) {
      out.add(code.trim());
    }
  }
  return [...out];
...
function extractConditionFingerprint(text) {
  ...
  const product = extractProductCode(text); if (product.length) parts.push(`product=${product.sort().join(',')}`);
