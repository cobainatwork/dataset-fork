...
function createPipeline({ embedding, vectorStore, clusterRepo, config, db }) {
  ...
  async function decideForQuestion({ questionId, vector, chunkId, projectId, candidateFingerprint, fingerprintGuardEnabled }) {
    ...
    const matchedQuestions = matchIds.length
      ...
      : [];
    // 單次 loop 同時建 cluster + fingerprint 映射，避免兩次 Object.fromEntries
    const clusterByQid = {};
    const fpByQid = {};
  ...
  async function processNewQuestion({ id, text, projectId, chunkId }) {
    ...
    // vector 計算結果丟棄、整個 processNewQuestion fail。
