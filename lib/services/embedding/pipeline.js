...
function createPipeline({ embedding, vectorStore, clusterRepo, config, db }) {
  ...
  async function decideForQuestion({ questionId, vector, chunkId, candidateFingerprint, fingerprintGuardEnabled }) {
    const matches = await vectorStore.search('question', vector, {
      top: MAX_NEIGHBORS,
      excludeIds: [questionId],
      excludeChunkId: chunkId || null,
    });
    ...
    const fpByQid = Object.fromEntries(matchedQuestions.map(q => [q.id, q.conditionFingerprint || null]));
  ...
  async function processNewQuestion({ id, text, projectId, chunkId }) {
    const candidateFingerprint = extractConditionFingerprint(text);
    await db.questions.update({
      where: { id },
      data: { conditionFingerprint: candidateFingerprint },
    });
    ...
    const decision = await decideForQuestion({
      questionId: id,
      vector,
      chunkId,
      candidateFingerprint,
      fingerprintGuardEnabled: config.fingerprintGuardEnabled,
    });
