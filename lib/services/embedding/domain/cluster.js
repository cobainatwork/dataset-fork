function decideClusterAssignment({ matches, threshold, currentClusterFor, fingerprintFor, candidateFingerprint, fingerprintGuardEnabled }) {
  if (!matches.length || matches[0].score < threshold) {
    return { action: 'create' };
  }
  let picked = matches[0];
  if (fingerprintGuardEnabled && candidateFingerprint) {
    picked = null;
    for (const m of matches) {
      if (m.score < threshold) break;
      const fp = fingerprintFor ? fingerprintFor(m.id) : null;
      if (!fp || fp === candidateFingerprint) { picked = m; break; }
    }
    if (!picked) return { action: 'create' };
  }
  const existing = currentClusterFor(picked.id);
  return {
    action: existing ? 'attach' : 'create',
    clusterId: existing || undefined,
    similarity: picked.score,
    matchedQuestionId: picked.id,
  };
}
