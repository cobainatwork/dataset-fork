...
const { processEmbeddingBackfillTask } = require('./embedding-backfill');
import { processClusterMaintenanceTask } from './cluster-maintenance';
import './recovery';
...
/**
 * 建立任務並非同步啟動處理（單一入口，供各 API route 收斂使用）
 * @param {Object} params
 * @param {string} params.projectId
 * @param {string} params.taskType
 * @param {Object|string} params.modelInfo
 * @param {string} params.language
 * @param {string} params.detail
 * @param {number} params.totalCount
 * @param {Object} params.note
 * @returns {Promise<object>} 建立後的任務 row
 */
export async function createTask({ projectId, taskType, modelInfo, language, detail = '', totalCount = 0, note }) {
  ...
  try {
    ...
    switch (task.taskType) {
      ...
      case 'embedding-backfill':
        ...
        break;
      case 'cluster-maintenance':
        await processClusterMaintenanceTask(task);
        break;
      default:
    ...
    // Fallback：若 handler 正常 return 但未自行 mark 完成/失敗，
    // 自動標記為 COMPLETED。既有 handlers (如 answer-generation) 自己
    // 設 status=1 後仍是 COMPLETED，此處不會覆寫。
    const finalTask = await prisma.task.findUnique({ where: { id: taskId }, select: { status: true } });
    if (finalTask && finalTask.status === TASK.STATUS.PROCESSING) {
      await updateTask(taskId, { status: TASK.STATUS.COMPLETED });
    }
  } catch (error) {
