...
export async function getTagById(id) {
  return await db.tags.findUnique({ where: { id } });
}
 ...
 */
export async function getTagsByParent(projectId, parentId) {
  return await db.tags.findMany({
...
/**
 * 重複標籤檢查（同一 parent 下同 label、排除指定 id）
 * @param {string} projectId
 * @param {string} label
 * @param {string|null} parentId
 * @param {string} excludeId
 */
export async function findTagByLabel(projectId, label, parentId, excludeId) {
  return await db.tags.findFirst({
    where: { projectId, label, parentId, id: { not: excludeId } }
  });
}
