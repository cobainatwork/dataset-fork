...
// 蒸餾問題專用文字塊名稱（distill route 經 ensureDistillChunk 共用）
const DISTILL_CHUNK_NAME = 'Distilled Content';

/**
 * 取得（不存在則建立）蒸餾專用文字塊
 * @param {string} projectId
 */
export async function ensureDistillChunk(projectId) {
  const existing = await getChunkByName(projectId, DISTILL_CHUNK_NAME);
  if (existing) return existing;
  return await db.chunks.create({
    data: {
      name: DISTILL_CHUNK_NAME,
      projectId,
      fileId: 'distilled',
