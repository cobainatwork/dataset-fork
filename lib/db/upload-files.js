...
import { removeClusterMember } from './questions';
...
export async function delUploadFileInfoById(fileId, opts = {}) {
  // opts.skipFsDelete=true 時不刪實體檔。用於 docver 同名替換：replace API
  // 已把新內容寫到「同 path/fileName」上，舊 row 清理時若仍 rm 會把新檔一起刪。
  const { skipFsDelete = false } = opts;
  try {
    // 1. 獲取檔案資訊
    ...
    if (chunkIds.length > 0) {
      ...
      // 獲取所有問題（含 clusterId，purge 後同步收斂 cluster 用）
    ...
    // 取代原 fire-and-forget maintenance：收斂改為同步 await（多數情況立即完成），
    // 但維持 best-effort 語意——單題收斂失敗（如 optimistic lock 重試耗盡）只記錄、
    // 不中斷 purge 與實體檔刪除；失敗的 cluster 會留 size drift，與原 maintenance
    // 失敗同級風險（不再有獨立 repair job 兜底）。
    for (const q of questions) {
      try {
        await removeClusterMember(q.id, q.clusterId);
      } catch (e) {
        console.error(`[purge] cluster convergence failed q=${q.id}`, e);
      }
    }

    // 7. 刪除檔案系統中的檔案（skipFsDelete=true 時略過，見函式頂註解）
    if (!skipFsDelete) {
...
export async function replaceUploadFile(oldFileId, newFileMeta) {
  ...
  //    skipFsDelete:true 因 replace API 已把新內容寫到同 path/fileName，
  //    這裡若再 rm 會把使用者剛上傳的新檔一起刪掉。
  const { stats } = await delUploadFileInfoById(oldFile.id, { skipFsDelete: true });
