...
/**
 * 執行遷移操作
 * @param {Object} task 任務物件，用於更新進度
 * @returns {Promise<number>} 遷移專案數量
 */
export async function main(task = null) {
  ...
  // 建立並行任務，保留原有的 Promise.all 高效並行處理
    ...
    try {
      ...
      if (!project) {
        ...
        if (projectDB) {
          ...
          return 1; // 返回1表示遷移了一個專案
        }
      }
      return 0;
    } catch (error) {
      console.error(`處理專案出錯：${projectPath}`, error);
      if (task) {
        task.errors.push({
          projectId: file.name,
          error: error.message
