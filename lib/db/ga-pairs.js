...
/**
 * 以 updates 全量取代某檔案的 GA pairs（單一事務：刪舊、驗證、建新、回讀）
 * @param {string} projectId
 * @param {string} fileId
 * @param {Array} updates - client 提交的 pair 陣列
 * @returns {Promise<Array>} - 取代後的新 GA pairs
 */
export async function replaceGaPairs(projectId, fileId, updates) {
  return await db.$transaction(async tx => {
    await tx.gaPairs.deleteMany({ where: { fileId } });
