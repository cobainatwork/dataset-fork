...
/**
 * 通用測評題目查詢（route 層不直接碰 Prisma 的入口）
 * @param {Object} args - Prisma findMany 引數（where/select/orderBy/skip/take...）
 */
export async function findEvalDatasets(args) {
  return await db.evalDatasets.findMany(args);
}

/**
 * 取單一測評題目（id 為 PK 時與 findUnique 同語意）
 * @param {Object} args - Prisma findFirst 引數
 */
export async function findFirstEvalDataset(args) {
  return await db.evalDatasets.findFirst(args);
}
 ...
 */
export async function countEvalDatasets(where) {
  return await db.evalDatasets.count({ where });
}
 ...
 * @param {Object} where - Prisma where 條件
 */
export async function groupEvalDatasetsByType(where) {
  return await db.evalDatasets.groupBy({
    by: ['questionType'],
    where,
    _count: { id: true }
  });
