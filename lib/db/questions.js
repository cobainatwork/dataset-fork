...
export async function updateQuestion(question) {
  try {
    const existing = await db.questions.findUnique({
      where: { id: question.id },
      select: { id: true, projectId: true, question: true },
    });
    const result = await db.questions.update({ where: { id: question.id }, data: question });
    if (existing && typeof question.question === 'string' && question.question !== existing.question) {
      // Question text changed: re-embed & re-evaluate cluster via existing pipeline.
      await enqueueDedup(existing.projectId, [existing.id], { force: true });
    }
    return result;
  } catch (error) {
...
/**
 * 同步清理 question 對應的 cluster 與 embedding 紀錄。
 * 必須在 question row 仍存在或剛刪除時呼叫，呼叫端要把 questionId 與
 * 它的 clusterId（刪除前先讀出）傳進來。
 */
async function cleanupQuestionEmbeddingAndCluster({ questionId, clusterId }) {
  const { decideClusterUpdateOnMemberRemoval } = await import(
    '@/lib/services/embedding/domain/cluster'
  );

  // 永遠先刪 Embeddings 對應筆，避免 orphan 向量
  await db.embeddings.deleteMany({
    where: { sourceType: 'question', sourceId: questionId },
  });

  if (!clusterId) return;

  const cluster = await db.questionCluster.findUnique({ where: { id: clusterId } });
  if (!cluster) return;

  const { shouldDelete, newSize } = decideClusterUpdateOnMemberRemoval({ size: cluster.size });

  if (shouldDelete) {
    await db.clusterFeedback.deleteMany({ where: { clusterId } });
    await db.clusterProject.deleteMany({ where: { clusterId } });
    await db.questionCluster.delete({ where: { id: clusterId } });
  } else {
    await db.questionCluster.update({
      where: { id: clusterId },
      data: { size: newSize, version: { increment: 1 } },
    });
  }
}
...
export async function deleteQuestion(questionId) {
  try {
    const q = await db.questions.findUnique({
      where: { id: questionId },
      select: { clusterId: true },
    });
    const result = await db.questions.delete({ where: { id: questionId } });
    await cleanupQuestionEmbeddingAndCluster({ questionId, clusterId: q?.clusterId });
    return result;
  } catch (error) {
    console.error('Failed to delete questions by id in database', String(error));
    throw error;
  }
}
...
export async function batchDeleteQuestions(questionIds) {
  try {
    ...
    const result = await db.questions.deleteMany({
      where: { id: { in: questionIds } },
    });
    for (const r of rows) {
      await cleanupQuestionEmbeddingAndCluster({ questionId: r.id, clusterId: r.clusterId });
    }
    return result;
  } catch (error) {
    console.error('Failed to delete batch questions in database', String(error));
    throw error;
  }
}
...
async function enqueueDedup(projectId, questionIds, options = {}) {
  if (!questionIds.length) return;
  try {
    const detail = options.force
      ? JSON.stringify({ questionIds, force: true })
...
/**
 * 通用 questions 查詢（route 層不直接碰 Prisma 的入口）
 * @param {Object} args - Prisma findMany 引數（where/select/orderBy...）
 */
export async function findQuestions(args) {
  return await db.questions.findMany(args);
}
...
export async function createQuestion(data) {
  return await db.questions.create({ data });
}

/**
 * 刪除某圖片的所有問題。
 * 必須走 cleanupQuestionEmbeddingAndCluster 不變量（先清 orphan
 * embedding、再處理 cluster size），否則會留下孤兒向量。
 * @param {string} imageId
 */
export async function deleteQuestionsByImageId(imageId) {
  const rows = await db.questions.findMany({
    where: { imageId },
    select: { id: true, clusterId: true },
  });
  if (rows.length === 0) return { count: 0 };
  const result = await db.questions.deleteMany({ where: { imageId } });
  for (const r of rows) {
    await cleanupQuestionEmbeddingAndCluster({ questionId: r.id, clusterId: r.clusterId });
  }
  return result;
