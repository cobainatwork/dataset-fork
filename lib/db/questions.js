...
/**
 * 收斂一個 question 離開後的 cluster（vector 清理 + size / primary / 刪除）。
 * 走 embedding service 的 ClusterManager：optimistic lock，與 pipeline 同紀律。
 * 契約：question row 已刪（呼叫端先刪 row 再呼叫），clusterId 為刪除前讀出的值。
 */
export async function removeClusterMember(questionId, clusterId) {
  const { getClusterManager } = await import('@/lib/services/embedding');
  await getClusterManager().removeMember({ questionId, clusterId });
}
...
export async function deleteQuestion(questionId) {
  try {
    ...
    await removeClusterMember(questionId, q?.clusterId);
    return result;
  } catch (error) {
...
export async function batchDeleteQuestions(questionIds) {
  try {
    ...
    for (const r of rows) {
      await removeClusterMember(r.id, r.clusterId);
    }
    return result;
  } catch (error) {
