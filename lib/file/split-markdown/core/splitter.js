...
function processSections(sections, outline, _minSplitLength, maxSplitLength) {
  ...
  for (const section of sections) {
    ...
    // 純標題、無內容、且 heading 也是空字串：完全沒東西可累積，跳過
    if (trimmedContent.length === 0 && !section.heading) continue;
    // 注意：trimmedContent 為空但 heading 非空時，仍要進入累積流程。
    // mineru 把 PDF 表格欄位轉成連續多個 `#### 欄位名` 短 heading，彼此之間
    // 無實質 paragraph content，這些 heading 文字本身就是表格資料、不能丟。
