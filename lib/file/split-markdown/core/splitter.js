 ...
 * Markdown 文件分割模組
 *
 * 策略 C+min-brake (合併到 max、邊界避開 trailing heading、min 煞車):
 *   - 累積多個 section 到一個 chunk，直到加入下一個 section 會超過
 *     maxSplitLength 才 push。同一份小文件 (總長 <= max) 不會被切。
 *   - **min-brake**：若 pending 已 >= minSplitLength 且下一個 section 自身
 *     也 >= minSplitLength，停止合併，即使候選長度尚未達 max。避免貪婪
 *     合併把多個完整章節擠進同一 chunk。下個 section 若 < min（碎片）
 *     仍會合併，避免不必要的小 chunk。
 ...
 *   - min 若 < 100 視為 sentinel (不啟用 brake)，沿用既有純 max 行為。
 */
...
function processSections(sections, outline, _minSplitLength, maxSplitLength) {
  ...
  // 預估整份大小（含 heading line 與 section 間隔）。若 ≤ max，整份塞得進
  // 一個 chunk，min-brake 不該介入——使用者設 max 較大就是表達「整份保留」。
  // brake 真正的設計用途是「整份 > max、不切會超」場景下避免貪婪 fill-to-max
  // 把多個完整章節擠進同 chunk。
  let totalEstimate = 0;
  for (const s of sections) {
    const c = (s.content || '').trim();
    if (!c.length) continue;
    const headingLen = s.heading
      ? (s.headingType === 'cn-numbered' ? s.heading.length + 1 : s.level + 1 + s.heading.length + 1)
      : 0;
    totalEstimate += headingLen + c.length + 2;
  }
  const skipBrake = totalEstimate <= maxSplitLength;

  // 整份 ≤ max：直接組 1 個 chunk 返回，跳過 brake/detachTrailingHeading 路徑，
  // 避免 trailing heading-like 短行被獨立 push 成迷你 chunk。
  if (skipBrake) {
    let bigContent = '';
    let firstHeadingSection = null;
    for (const s of sections) {
      const c = (s.content || '').trim();
      if (!c.length) continue;
      const headingLine = s.heading
        ? (s.headingType === 'cn-numbered'
            ? `${s.heading}\n`
            : `${'#'.repeat(s.level)} ${s.heading}\n`)
        : '';
      bigContent += (bigContent ? '\n\n' : '') + headingLine + c;
      if (!firstHeadingSection && s.heading) firstHeadingSection = s;
    }
    if (!bigContent.trim()) return [];
    const metaForSummary = firstHeadingSection || { heading: null, level: 0 };
    const chunkSummary = summary.generateEnhancedSummary(metaForSummary, outline);
    return [{ summary: chunkSummary, content: bigContent.trim() }];
  }
  ...
  for (const section of sections) {
    ...
    // 中文編號 heading（section.headingType === 'cn-numbered'）已含完整文字，
    // 不需要加 # 前綴；markdown # heading 維持原本重建邏輯。
    const headingLine = section.heading
      ? (section.headingType === 'cn-numbered'
          ? `${section.heading}\n`
    ...
    // min-brake：當 min 為實際門檻（>=100）且 pending 已達 min、且即將
    // 加入的 sectionBlock 自身也達 min，停止合併避免貪婪 fill-to-max 把
    // 多個完整章節擠進同 chunk。sectionBlock < min（碎片）仍會合併。
    const candidate = pending ? `${pending}\n\n${sectionBlock}` : sectionBlock;
    const minBrakeFires =
      !skipBrake &&
      _minSplitLength >= 100 &&
      pending.length >= _minSplitLength &&
      sectionBlock.length >= _minSplitLength;
    if (!minBrakeFires && candidate.length <= maxSplitLength) {
      pending = candidate;
