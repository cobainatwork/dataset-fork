...
import { getChunkByProjectId, saveChunks, deleteChunksByFileId } from '@/lib/db/chunks';
const {
  TokenTextSplitter,
...
export async function splitProjectFile(projectId, file) {
  ...
  try {
    ...
    // Idempotent：先清同 fileId 既有 chunks（含 cascade 清 questions），避免
    // file-processing task 被 recovery 重跑或重複觸發時建出 part-N 重複行。
    // 正常 upload 路徑下該 fileId 還沒 chunks → deleteChunksByFileId 直接 no-op。
